...
@Injectable()
@Scene(mainScenes.SUBSTATION_UPDATE_SCENE)
export class SubstationsUpdateScene {
  ...
  async substationStart(@Ctx() ctxScene: SceneContext, @Ctx() ctx: Context) {
    ...
    await ctxScene.reply('Для обновления ТП введите её номер:', kbBtnCancel);
    return;
  }
  ...
  async textHandler(
    ...
    if (ctx.session.mainEvent === mainEvents.SUBSTATION_COORDINATES) {
      ...
      const substationDto: UpdateSubstationDto = {
        latitude,
        longitude,
      };
      const substation = await this.substationsService.update(
        ctx.session.substation_name_value,
        substationDto,
      );
      if (!substation) {
